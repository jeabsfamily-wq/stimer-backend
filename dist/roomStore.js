...
export function onClientDisconnect(room, clientId) {
    const sid = room.bindings.get(clientId);
    if (sid) {
        const slot = room.stations.get(sid);
        if (slot)
            slot.connected = false; // keep owner/binding
    }
...
export function onAnyClientJoin(room) {
...
export function createRoom(centralClientId, stationsCount, roundDurationSec) {
    const code = genCode();
    const room = {
        code,
        centralClientId,
...
export function serialize(room) {
    return {
        ...
        timeLeft: room.timeLeft ??
            (room.startedAt
                ? Math.max(0, room.roundDurationSec -
                    Math.floor((Date.now() - room.startedAt) / 1000))
                : undefined),
    };
}
export function claimStation(room, clientId, stationId) {
...
export function allClaimedAndReady(room) {
...
export function updateConfig(room, stationsCount, roundDurationSec) {
    ...
    for (let i = stationsCount + 1; i <= room.stationsCount; i++) {
        const s = room.stations.get(i);
        if (s?.ownerClientId)
            throw new Error('E_STATIONS_IN_USE');
    }
    room.stationsCount = stationsCount;
    for (let i = stationsCount + 1; i <= 200; i++)
...
export function tick(room, onWarn30, onTimeUp) {
    if (room.state !== 'RUNNING' || room.startedAt == null)
        return;
...
export function scheduleRoomCleanupIfEmpty(room) {
    if (isRoomEmpty(room) && !emptyRoomTimers.has(room.code)) {
        const t = setTimeout(() => {
            deleteRoom(room.code);
            emptyRoomTimers.delete(room.code);
